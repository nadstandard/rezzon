...
export function AliasesView() {
  ...
  const disconnectLibrary = useAppStore((state) => state.disconnectLibrary);
  ...
  return (
    <>
                        ...
                        <tr key={key} className={`alias-row ${isSelected ? 'selected' : ''}`} onClick={() => setSelectedAliasDetail(alias)}>
                          ...
                          <td onClick={(e) => e.stopPropagation()}>
                            <div className="action-btns">
                              <button className="action-btn" onClick={() => setSelectedAliasDetail(alias)} title="View details"><Eye className="icon sm" /></button>
                              <button 
                                className="action-btn action-btn--danger" 
                                onClick={() => {
                                  if (alias.type === 'external' && activeLibrary) {
                                    handleDisconnect(alias.targetLibrary, 1);
                                  }
                                }}
                                title={alias.type === 'external' ? 'Disconnect' : 'Remove alias'}
                              >
                                <Link2Off className="icon sm" />
                              </button>
                            </div>
      ...
      {/* Detail Panel */}
      {selectedAliasDetail && activeLibrary && (
        <AliasDetailPanel 
          alias={selectedAliasDetail} 
          library={activeLibrary} 
          allLibraries={libraries} 
          onClose={() => setSelectedAliasDetail(null)}
          onDisconnect={(libraryName) => {
            const ext = connectedExternals.find(e => e.libraryName === libraryName);
            if (ext) {
              handleDisconnect(libraryName, ext.aliasCount);
            }
          }}
          onRemoveAlias={(variableId, modeId) => {
            removeAlias(activeLibrary.id, variableId, modeId);
            setSelectedAliasDetail(null);
          }}
        />
      )}
      ...
      {restoreModal && (
        <RestoreModal
          ...
          onRestore={() => { 
            restoreLibrary(restoreModal.library.libraryName); 
...
function AliasDetailPanel({ 
  ...
  onClose: () => void;
  onDisconnect?: (libraryName: string) => void;
  onRemoveAlias?: (variableId: string, modeId: string) => void;
}) {
  ...
  return (
    <aside className="panel" style={{ width: 300 }}>
      ...
      <div style={{ padding: '12px 14px', borderTop: '1px solid var(--border)', display: 'flex', gap: 8 }}>
        {alias.type === 'broken' ? (
          <>
            <button className="btn btn--primary" style={{ flex: 1 }} title="Select new target variable"><Link className="icon sm" /> Fix</button>
            <button 
              className="btn btn--danger" 
              onClick={() => onRemoveAlias?.(alias.sourceVariableId, alias.modeId)}
              title="Remove broken alias"
            >
              <Link2Off className="icon sm" />
            </button>
          </>
          ...
          <>
            <button className="btn btn--ghost" style={{ flex: 1 }} title={alias.type === 'external' ? 'View target in external library' : 'Change alias target'}>
              <Eye className="icon sm" /> {alias.type === 'external' ? 'View' : 'Change'}
            </button>
            <button 
              className="btn btn--ghost" 
              style={{ color: 'var(--red)' }}
              onClick={() => {
                if (alias.type === 'external' && onDisconnect) {
                  onDisconnect(alias.targetLibrary);
                } else if (onRemoveAlias) {
                  onRemoveAlias(alias.sourceVariableId, alias.modeId);
                }
              }}
              title={alias.type === 'external' ? 'Disconnect external library' : 'Remove alias'}
            >
              <Link2Off className="icon sm" />
            </button>
          </>
