...
export function VariablesView() {
  ...
  // Wszystkie ID zmiennych po filtrach (nie tylko widoczne w rozwiniętych folderach)
  ...
  // Row click with shift support - używamy WSZYSTKICH zmiennych po filtrach
  const handleRowClick = useCallback((id: string, isVariable: boolean, e: React.MouseEvent) => {
    if (!isVariable) return;
    
    if (e.shiftKey && lastClickedId) {
      // Shift+click - select range (używamy wszystkich zmiennych po filtrach)
      const startIdx = allFilteredVariableIds.indexOf(lastClickedId);
      const endIdx = allFilteredVariableIds.indexOf(id);
      if (startIdx !== -1 && endIdx !== -1) {
        const start = Math.min(startIdx, endIdx);
        const end = Math.max(startIdx, endIdx);
        const rangeIds = allFilteredVariableIds.slice(start, end + 1);
  ...
  }, [lastClickedId, allFilteredVariableIds, selectedVariables, selectVariables, toggleVariable]);
